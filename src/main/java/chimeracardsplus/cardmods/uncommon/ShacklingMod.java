            ...
            public void update() {
                if (cardTarget != null) {
                    if (!cardTarget.hasPower(ArtifactPower.POWER_ID)) {
                        this.addToTop(new ApplyPowerAction(cardTarget, AbstractDungeon.player, new GainStrengthPower(cardTarget, 5), 5, true, AttackEffect.NONE));
                    }
                    this.addToTop(new ApplyPowerAction(cardTarget, AbstractDungeon.player, new StrengthPower(cardTarget, -5), -5, true, AttackEffect.NONE));
                }
                this.isDone = true;
            }
        });
