            ...
            public void update() {
                for (int i = AbstractDungeon.getMonsters().monsters.size(); i-- > 0; ) {
                    ...
                    if (!mo.hasPower("Artifact")) {
                        this.addToTop(new ApplyPowerAction(mo, AbstractDungeon.player, new GainStrengthPower(mo, 3), 3, true, AttackEffect.NONE));
                    }
                }
                for (int i = AbstractDungeon.getMonsters().monsters.size(); i-- > 0; ) {
                    AbstractMonster mo = AbstractDungeon.getMonsters().monsters.get(i);
                    this.addToTop(new ApplyPowerAction(mo, AbstractDungeon.player, new StrengthPower(mo, -3), -3, true, AttackEffect.NONE));
                }
